...
describe("P4 — phone contact system", () => {
  it("registry has the mother contact", () => {
    expect(PHONE_CONTACTS.map((c) => c.id).sort()).toEqual(["mother"]);
...
describe("P4 — scheduled world events", () => {
  ...
  it("evidence-triggered event fires on discovery", () => {
    let s = fresh();
    s = { ...s, evidenceIds: [...s.evidenceIds, "ev_source_post"] };
    const step = applyWorldEvents(s);
    expect(step.fired.map((e) => e.id)).toContain("ev_evidence_source_post");
  });
